# Migrating Expo UI to VJRA Design System

Read `artifacts/vjra-ds/docs/AGENTS.md` and
`artifacts/vjra-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/vjra-ds/lib/native-theme` and
  `@workspace/vjra-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/vjra-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/vjra-ds` and retains only
